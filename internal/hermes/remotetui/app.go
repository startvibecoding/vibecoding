...
// Options configures the Hermes remote TUI.
type Options struct {
	URL       string
	SessionID string
	AuthToken string
...
// NewApp creates a new Hermes remote TUI application.
func NewApp(opts Options) *App {
	...
	if opts.SessionID != "" {
		...
		wsURL += sep + "session=" + opts.SessionID
	}
...
func (a *App) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case tea.KeyEnter:
			...
			if a.waitingForApproval {
				...
				_ = a.sendWS(wsMessage{Type: "approval", ApprovalID: a.pendingApprovalID, Approved: approved})
				if approved {
			...
			if a.waitingForQuestion {
				...
				if _, err := fmt.Sscanf(answer, "%d", &num); err == nil && num > 0 {
					_ = a.sendWS(wsMessage{Type: "question", QuestionID: a.pendingQuestionID, Answer: answer})
	...
	case agentStartMsg:
		...
		return a, tea.Batch(a.tickSpinner(), a.timer.Reset(), a.timer.Start())

	case agentEventMsg:
		return a, a.handleAgentEvent(msg.event)

	case wsEventMsg:
		if msg.event.Type == "connected" {
			if msg.event.Model != "" {
				a.modelName = msg.event.Model
			}
			if msg.event.WorkDir != "" {
				a.workDir = msg.event.WorkDir
			}
		}
		return a, a.handleAgentEvent(msg.event.agentEvent())
